        ...
        <div class="recommendation">
            ...
            <div v-else class="recommendation-list">
                <div v-for="item in currentRecommendations" :key="item.osm_id" class="recommendation-card"
                    ...
                    <p class="category">分类：{{ item.fclass }}</p>
    ...
    methods: {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    if (Array.isArray(this.recommendations)) {
                        ...
                        if (index !== -1) {
                            this.$set(this.recommendations, index, updatedItem);
                        }
                    }
                }
        ...
        onSearch() {
            ...
            const params = {
                filter: this.filter,
                query: this.query,
                category: this.categoryFilter // 新增分类筛选参数
            };

            axios.get('http://localhost:3000/api/recom', { params })
                .then(response => {
                    this.filteredRecommendations = response.data.map(item => ({
                        name: item.name,
                ...
                .catch(err => {
                    if (err.response) {
                        console.error(`请求失败: ${err.response.status} ${err.response.statusText}`);
                        this.error = `请求失败: ${err.response.status} ${err.response.statusText}`;
                    } else if (err.request) {
                        console.error('请求没有收到响应:', err.request);
                        this.error = '请求没有收到响应，请稍后重试';
                    } else {
